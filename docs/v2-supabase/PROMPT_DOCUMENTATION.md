...
```
- /docs/v2-supabase/FLOW.md
- /docs/v2-supabase/ARCHITECTURE.md
- /docs/CHANGELOG.md
```
...
```
Added password reset flow documentation to FLOW.md.
Updated ARCHITECTURE.md to include new API endpoint.
Recorded change in CHANGELOG.md.
```

### 3. Updated Content

Show the actual changes (use diff format if helpful):
```markdown
## 9. FLOW 9 – PASSWORD RESET
...
How to verify docs are correct:
```
1. Check that password reset endpoint exists in backend/src/routes/auth.routes.js
2. Verify flow matches actual implementation
3. Confirm all cross-references work
```
...
ACCESS_SECURITY.md         (Security model)
FLOW.md                    (Business flows)
    ↓
DEV_PLAYBOOK.md            (Developer guide)
ANTIGRAVITY_RULES.md       (AI coding rules)
```

### Cross-Reference Examples

```markdown
See [ANTIGRAVITY_RULES.md](./ANTIGRAVITY_RULES.md) for complete rules.
See [Rule 06: Password Hashing](./ANTIGRAVITY_RULES.md#rule-06-password-hashing).
See [ARCHITECTURE.md - Security Architecture](./ARCHITECTURE.md#6-security-architecture).
For historical context, see [v1 Archive](../v1-gsheet-archive/README.md).
```
...
**Copy everything above this line and paste to Antigravity before requesting documentation updates.**
